...
class CityNameNotFoundException(Exception):
    pass


def has_forecast_expired(forecast):
    return (
        dt.datetime.now(tz=dt.timezone.utc) - forecast.updated_at
    ).seconds > settings.FORECAST_EXPIRATION_TIME_IN_SECONDS


def get_weather_api_response(city):
    for obj in objects:
        if obj["name"].lower() == city.lower():
            break
    else:
        raise CityNameNotFoundException(
            f"No city with the name {city} has been found."
        )

    return requests.get(
        settings.WEATHER_API_URL.format(
            lat=obj["coords"]["lat"],
            lon=obj["coords"]["lon"]
        ),
        headers=settings.WEATHER_API_REQUEST_HEADERS
    )


def format_forecast_object(obj):
    return {
...
def collect_information_from_weather_api(city):
    try:
        response = get_weather_api_response(city)
        response.raise_for_status()
        forecast = update_or_create_forecast(city, response)
    except CityNameNotFoundException:
        return JsonResponse({
            "__status": "error",
            "__message": _("City with the given name has not been found."),
        }, status=HTTPStatus.BAD_REQUEST.value)
    except requests.HTTPError:
        return JsonResponse({
            "__status": "error",
            "__message": _("Our partner's weather API has not been able to handle your request.")
        }, status=HTTPStatus.BAD_REQUEST.value)
    except Exception:
        return JsonResponse({
            "__status": "error",
            "__message": _("Our server has experienced an unexpected failure.")
        }, status=HTTPStatus.INTERNAL_SERVER_ERROR.value)

    return JsonResponse({
        "__status": "success",
        "__message": _("Successfully collected information about the target city."),
        **format_forecast_object(forecast)
    })


def update_or_create_forecast(city, response):
    _ = response.json()

    with transaction.atomic():
        c, __ = City.objects.get_or_create(name=city)
        forecast, __ = Forecast.objects.update_or_create(
            city=c,
            defaults={
                "temperature": _["fact"]["temp"],
                "pressure": _["fact"]["pressure_mm"],
                "wind_speed": _["fact"]["wind_speed"]
            }
        )
        return forecast
